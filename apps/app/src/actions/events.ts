...
import { generateUniqueEventSlug } from "@evenly/core/events";
...
export async function createEventAction(
	...
	if (!parsed1.success) {
		return {
			...
			fieldErrors: parsed1.error.flatten().fieldErrors,
		};
	}
	...
	if (!parsed2.success) {
		return {
			error: "Données de lieu invalides.",
			fieldErrors: parsed2.error.flatten().fieldErrors,
		};
	}
	...
	const endsAtDate =
		endsAt && endsAt.length > 0 && endsAtTime && endsAtTime.length > 0
			? new Date(`${endsAt}T${endsAtTime}`)
			: null;
...
const updateSettingsSchema = z.object({
	...
	refundPolicy: z
		.enum(["NON_REFUNDABLE", "ORGANIZER_DEFINED", "ALWAYS_REFUNDABLE"])
		.optional(),
