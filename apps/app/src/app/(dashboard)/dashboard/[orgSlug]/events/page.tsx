...
export default async function EventsPage({
	...
	const events = await db.event.findMany({
		...
		include: {
			ticketTypes: {
				select: { priceCents: true, quantitySold: true, quantity: true },
			},
			...
			{events.length === 0 ? (
				<div className="text-center py-20 border-2 border-dashed border-gray-200 rounded-2xl">
					<div className="w-16 h-16 bg-violet-50 rounded-2xl flex items-center justify-center mx-auto mb-4">
						...
						>
							<path
								...
								strokeLinejoin="round"
								strokeWidth={1.5}
								d="M15 5v2m-6-2v2M7 5a2 2 0 00-2 2v11a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2H7z"
							/>
						</svg>
					</div>
					...
					<p className="text-sm text-gray-500 mb-6">
						Créez votre premier événement en moins de 60 secondes.
					</p>
					<Link
						href={`/dashboard/${orgSlug}/events/new`}
				...
				<div className="space-y-3">
					{events.map((event) => {
						const revenue = event.ticketTypes.reduce(
							(acc, tt) => acc + tt.priceCents * tt.quantitySold,
							0,
						);
						const totalCapacity = event.ticketTypes.reduce(
							(acc, tt) => acc + (tt.quantity ?? 0),
							0,
						);
						const totalSold = event.ticketTypes.reduce(
							(acc, tt) => acc + tt.quantitySold,
							0,
						);
						const fillRate =
							totalCapacity > 0
						...
						return (
							...
							>
								...
								<svg
									className="w-4 h-4 text-gray-300 group-hover:text-violet-400 flex-shrink-0 transition-colors"
									fill="none"
									viewBox="0 0 24 24"
									stroke="currentColor"
								>
