    ...
    include: {
      event: {
        select: {
          ...
          locationName: true, locationAddress: true, locationType: true,
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-lg mx-auto px-4 py-8 space-y-5">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-6 text-center space-y-2">
          ...
          <h1 className="text-lg font-bold text-gray-900">Paiement confirmé !</h1>
        ...
        {event.confirmationMessage && (
          <div className="bg-violet-50 rounded-2xl border border-violet-100 p-4">
            <p className="text-sm text-violet-800">{event.confirmationMessage}</p>
          </div>
        )}
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-5 space-y-3">
          ...
          <div className="pt-2 border-t border-gray-100 flex items-center justify-between">
            <div className="text-sm">
              <span className="text-gray-500">{order.items.reduce((a, i) => a + i.quantity, 0)} billet(s)</span>
            </div>
        ...
        {firstTicketQr && (
          <div className="bg-white rounded-2xl border border-gray-200 p-5 flex items-center gap-4">
            <img src={firstTicketQr} alt="QR Code" width={80} height={80} className="rounded-lg border border-gray-100 flex-shrink-0" />
            <div>
              <p className="text-sm font-semibold text-gray-900">Votre billet</p>
              <p className="text-xs text-gray-500 mt-0.5">
                {order.tickets.length > 1 ? `+${order.tickets.length - 1} autre(s) billet(s)` : "Présentez ce QR code à l'entrée"}
              </p>
              <a href={ticketsUrl} className="text-xs text-violet-600 hover:underline mt-1 inline-block">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-4 space-y-3">
          <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Partager l'événement</p>
        ...
        <p className="text-center text-xs text-gray-400 pb-4">
          ...
          <a href={`/refund/${order.magicToken}`} className="hover:underline">Demander un remboursement</a>
        </p>
      </div>
    </div>
  );
