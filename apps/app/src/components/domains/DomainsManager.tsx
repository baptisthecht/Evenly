...
export function DomainsManager({ org, customDomains: initialDomains }: Props) {
  ...
  const [verifyResults, setVerifyResults] = useState<Record<string, { verified: boolean; error?: string | null }>>({});
    ...
    startTransition(async () => {
      ...
      if ("verified" in result) {
        setVerifyResults((prev) => ({
          ...prev,
          [domainId]: { verified: result.verified!, error: result.error ?? undefined },
        }));
        if (result.verified) {
          setDomains((prev) =>
