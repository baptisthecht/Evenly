...
const features = [
			...
			<div className="bg-white rounded-2xl border border-black/8 p-5 space-y-3 shadow-sm">
				{["Infos de base", "Lieu", "Récapitulatif"].map((step, i) => (
					<div key={step} className="flex items-center gap-3">
						<div
							className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${i === 0 ? "bg-[var(--violet)] text-white" : "bg-black/5 text-[var(--muted)]"}`}
						>
							{i + 1}
						</div>
						<span
							className={`text-sm ${i === 0 ? "font-semibold text-[var(--ink)]" : "text-[var(--muted)]"}`}
						>
							{step}
						</span>
						{i === 0 && (
			...
			<div className="bg-black rounded-2xl p-5 space-y-3 text-center shadow-lg">
				...
				<div className="flex gap-1 justify-center mt-2">
					{["scan", "manuel", "stats"].map((t) => (
						<div
							key={t}
							className={`flex-1 text-center py-1.5 rounded-lg text-[10px] ${t === "scan" ? "bg-violet-600 text-white" : "text-gray-500"}`}
						>
							{t === "scan"
								? "📷 Scanner"
								: t === "manuel"
			...
			<div className="bg-white rounded-2xl border border-black/8 p-5 shadow-sm space-y-3">
				{[
					...
					<div key={a.label} className="flex items-center gap-3">
						...
						<span
							className={`text-xs px-2 py-0.5 rounded-full ${a.status === "Actif" ? "bg-green-50 text-green-700" : "bg-gray-50 text-gray-400"}`}
						>
							{a.status}
						</span>
					</div>
	...
	},
	{
		...
		visual: (
			<div className="bg-white rounded-2xl border border-black/8 p-5 shadow-sm space-y-3">
				{[
					{ domain: "mon-asso.evoly.me", status: "✅ Actif", plan: "Free" },
					{
						domain: "tickets.monsite.com",
						status: "✅ SSL actif",
						plan: "Pro",
					},
				].map((d) => (
					<div key={d.domain} className="space-y-0.5">
						<div className="flex items-center gap-2">
							...
							<span
								className={`text-[10px] px-1.5 py-0.5 rounded font-medium ${d.plan === "Pro" ? "bg-[var(--violet-subtle)] text-[var(--violet)]" : "bg-gray-100 text-gray-500"}`}
							>
								{d.plan}
							</span>
						</div>
...
export function Features() {
	return (
		<section
			className="py-16 px-4 bg-white border-y border-black/5"
			id="features"
		>
