...
const columns = [
  ...
  },
  {
    ...
    links: [
      ...
      { label: "Contact", href: "mailto:hello@evenly.com" },
      { label: "Status", href: "#" },
    ],
  },
];
      ...
      <div className="max-w-5xl mx-auto">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mb-10">
          ...
          <div className="col-span-2 md:col-span-1 space-y-4">
            <span className="font-display text-xl text-white">evenly</span>
          ...
          {columns.map((col) => (
            <div key={col.title} className="space-y-3">
              <p className="text-xs font-semibold text-white uppercase tracking-wider">{col.title}</p>
              <ul className="space-y-2">
                {col.links.map((link) => (
                  <li key={link.label}>
                    <a href={link.href} className="text-xs hover:text-white transition-colors">
                      {link.label}
                    </a>
                  </li>
                ))}
        ...
        <div className="border-t border-white/5 pt-6 flex flex-col sm:flex-row items-center justify-between gap-3">
          <p className="text-xs">© 2025 Evenly. Fait avec ♥ en Belgique.</p>
          <a
