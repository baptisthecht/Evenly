    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-[var(--sand)]/90 backdrop-blur-md border-b border-black/5">
      <nav className="max-w-6xl mx-auto px-4 h-14 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-6 text-sm text-[var(--muted)]">
          <a href="#features" className="hover:text-[var(--ink)] transition-colors">Fonctionnalités</a>
          <a href="#pricing" className="hover:text-[var(--ink)] transition-colors">Tarifs</a>
          <a href="#faq" className="hover:text-[var(--ink)] transition-colors">FAQ</a>
        </div>

        {/* CTA */}
        <div className="hidden md:flex items-center gap-3">
          <a href={`${APP_URL}/login`} className="text-sm text-[var(--muted)] hover:text-[var(--ink)] transition-colors px-3 py-1.5">
            Connexion
          </a>
          <a
        ...
        <div className="md:hidden border-t border-black/5 bg-[var(--sand)] px-4 py-4 space-y-3">
          {["#features", "#pricing", "#faq"].map((href) => (
            <a key={href} href={href} onClick={() => setOpen(false)}
              ...
              {href === "#features" ? "Fonctionnalités" : href === "#pricing" ? "Tarifs" : "FAQ"}
            </a>
          ))}
          <div className="pt-2 flex flex-col gap-2">
            <a href={`${APP_URL}/login`} className="text-sm text-center border border-black/10 rounded-full py-2 hover:bg-black/5">
              Connexion
            </a>
            <a href={`${APP_URL}/register`} className="text-sm text-center bg-[var(--ink)] text-white rounded-full py-2 hover:bg-[var(--violet)] transition-colors">
              Créer un compte
            </a>
          </div>
        </div>
