      ...
      <Body style={body}>
        <Container style={container}>
          ...
          <Section style={{ textAlign: "center", marginBottom: 24 }}>
            ...
            <Text style={subtitle}>Bonjour {buyerName}, vos billets sont prêts.</Text>
          </Section>
          ...
          <Section style={card}>
            ...
            <Row
              label="💳 Total"
              value={totalCents === 0 ? "Gratuit" : `${(totalCents / 100).toFixed(2)}€`}
            />
          </Section>

          {/* Custom message */}
          {confirmationMessage && (
            <Section style={{ ...card, backgroundColor: "#f5f3ff", border: "1px solid #ede9fe" }}>
              <Text style={{ fontSize: 14, color: "#6d28d9", margin: 0, lineHeight: "1.6" }}>
                {confirmationMessage}
              </Text>
            </Section>
          )}
...
function Row({ label, value }: { label: string; value: string }) {
  return (
    <Section style={{ display: "flex", justifyContent: "space-between", padding: "4px 0" }}>
      <Text style={{ fontSize: 14, color: "#6b7280", margin: 0 }}>{label}</Text>
      <Text style={{ fontSize: 14, color: "#111827", fontWeight: 500, margin: 0 }}>{value}</Text>
    </Section>
  );
}
